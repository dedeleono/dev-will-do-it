...
impl TradingPlatform {
    ...
    pub fn deposit(&mut self, signer: &str, amount: u64) -> Result<Tx, ApplicationError> {
        match self.accounts.deposit(signer, amount) {
            Ok(tx) => {
    ...
    pub fn withdraw(&mut self, signer: &str, amount: u64) -> Result<Tx, ApplicationError> {
        match self.accounts.withdraw(signer, amount) {
            Ok(tx) => {
                self.tx_log.push(tx.clone());
                Ok(tx)
            },
    ...
    pub fn send(
        ...
        match self.accounts.send(sender, recipient, amount) {
            Ok((send_tx, receive_tx)) => {
                self.tx_log.push(send_tx.clone());
                self.tx_log.push(receive_tx.clone());
                Ok((send_tx, receive_tx))
            },
            Err(e) => Err(e),
        }
    ...
    pub fn order(&mut self, order: Order) -> Result<Receipt, ApplicationError> {
        let order_total = order.amount * order.price;
        match self.balance_of(&order.signer) {
            Ok(balance) if &order.side == &Side::Buy && balance < &order_total => {
                return Err(ApplicationError::AccountUnderFunded(order.signer, order_total))
            }
            Ok(_) => {}
            Err(e) => return Err(e),
        }
        ...
        let result: Result<Vec<_>, ApplicationError> = receipt
            .matches
            .iter()
            .map(|m| {
                match m.side {
                    Side::Buy => self.send(&m.signer, &signer, m.amount * m.price),
                    Side::Sell => self.send(&signer, &m.signer, m.amount * m.price),
                }
                .map(|(t1, t2)| vec![t1, t2])
            })
            .collect();
        self.tx_log.extend(result?.into_iter().flatten());
        Ok(receipt)
    }
}
