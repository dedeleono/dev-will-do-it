        ...
        'outer: while remaining_amount > 0 {
            ...
            match orderbook_entry.next() {
                Some((price, orderbook_entry)) => {
                    ...
                    // pop the Order with lowest sequence off the heap
                    'ask_loop: while let Some(mut pos) = orderbook_entry.pop() {
                        // check for self matching and whether the current `pos` order sufficiently covers the `order`
                        if pos.signer == order.signer {
                            self_matches.push(pos);
                            continue 'ask_loop;
                        }

                        // using checked_sub guards against overflow errors
                    ...
                    // Return the self-matched orders at their price in the book. 
                    // To avoid an infinite loop, we put the orders back after finishing the matching for this price point
                    self_matches
                        .into_iter()
...
mod tests {
    ...
    fn test_MatchingEngine_process_partially_match_order() {
        ...
        // No matches yet, order book was empty before
        ...
        // There is one match in the receipt's matches
        // A `PartialOrder` with Alice's properties
        // 0 remaining in Sell side
        assert_eq!(bob_receipt.matches[0], 
            PartialOrder {
                price: 10,
                amount: 1,
                remaining: 0,
        ...
        let (_, heap) = matching_engine
            .bids
            .first_key_value()
            .unwrap();

        let bob_partial_amount = heap.peek().unwrap().amount;

        // Bob's order ordinal
        assert_eq!(bob_receipt.ordinal, 2);
        // After partial match, there is no order left on the asks side of the book
        assert!(matching_engine.asks.is_empty());
        // `PartialOrder` is now in the bids side of the book with 1 remaining in amount
        assert_eq!(bob_partial_amount, 1);
    }
